...
(** This is an alternate version of [Norm.v] that is used as an
    example in [Extraction2.v].  It is not intended to be read on its
    own. *)
...
Definition closed (t:tm) :=
  forall x, appears_free_in x t -> False.
...
Lemma context_invariance : forall Gamma Gamma' t S,
     ...
     has_type Gamma' t S.
Proof with eauto.
  intros Gamma Gamma' t S H H0. generalize dependent Gamma'.
...
Definition halts  (t:tm) : Type :=  (exists t' : tm, (t ==>* t') * value t') %type.
...
Lemma subst_not_afi : forall t x v, closed v -> (appears_free_in x ([x:=v]t) -> False).
...
Proof. 
  ...
  Case "T_True".
    ...
    unfold R. split. 
    apply T_True.
    split.  unfold halts.  exists ttrue. split. apply multi_refl. apply v_true. auto.

  Case "T_False".
    rewrite msubst_false.
    unfold R. split. 
    apply T_False.
    split.  unfold halts.  exists tfalse. split. apply multi_refl. apply v_false. auto.
...
Proof with eauto.
  ...
  t_cases (induction t) Case; intros Gamma T Htc; inversion Htc; clear Htc.
  Case "tvar"...
  Case "tapp".
